    ...
    <html lang="es" className="h-full">
      <body className="min-h-screen flex flex-col h-full bg-slate-50 text-slate-900 antialiased">
              ...
              {links.map((link) => {
                ...
                return (
                  <Link
                    key={link.href}
                    href={link.href}
                    className={`relative px-3 py-2 font-medium cursor-pointer transition-colors duration-200 ${isActive ? "text-emerald-700" : "text-slate-700 hover:text-emerald-600"}`}>
                    {link.label}

                    <span
                      className={`absolute left-0 -bottom-1 h-[2px] bg-emerald-600 transition-all duration-300 ${isActive ? "w-full" : "w-0"}`}
                    />
                  </Link>
                );
              })}
        ...
        <main className="flex-1 w-full">
          {/* Contenedor centrado y con max-width — separado del main para que main pueda estirarse */}
          <div className="max-w-5xl mx-auto w-full p-4">
            {children}
          </div>
        </main>

        {/* mt-auto empuja el footer hacia abajo si hay espacio */}
        <footer className="mt-auto w-full bg-white border-t">
          <div className="max-w-5xl mx-auto px-4 py-3 text-sm text-slate-500 flex flex-col gap-2 sm:flex-row sm:justify-between sm:items-center">
            <span className="text-center sm:text-left">© 2025 Ezequiel Arce</span>
